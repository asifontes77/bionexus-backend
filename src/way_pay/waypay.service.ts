...
export class WaypayService {
  ...
  async getInvoiceTotales(admission: Date, idUser: number, idTypePay: number) {
    const totales = await this.waypayRepository
      ...
      .select([
        'COALESCE(SUM(CASE WHEN way_pay.annulment = false THEN way_pay_items.localEquivalentAmount ELSE 0 END), 0) AS total_not_annulled',
        'COALESCE(SUM(CASE WHEN way_pay.annulment = true THEN way_pay_items.localEquivalentAmount ELSE 0 END), 0) AS total_annulled',
        'COALESCE(SUM(CASE WHEN way_pay.annulment = false THEN way_pay_items.baseAmount ELSE 0 END), 0) AS total_dollars_not_annulled',
        'COALESCE(SUM(CASE WHEN way_pay.annulment = true THEN way_pay_items.baseAmount ELSE 0 END), 0) AS total_dollars_annulled',
      ])
