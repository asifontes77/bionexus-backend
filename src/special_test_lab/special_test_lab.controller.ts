...
export class SpecialtestlabController {
  ...
  @Post()
  createSpecialTestLab(request: SecurityAuthenticatedRequest, @Body() body: CreateSpecialTestLabDto) {
    return this.service.createSpecialTestLab(body, getSecurityAuditActorUserId(request) ?? undefined);
  }

  @UseGuards(JwtUserGuard)
  @Patch(':id')
  async updateSpecialTestLab(request: SecurityAuthenticatedRequest, @Param('id', ParseIntPipe) id: number, @Body() body: UpdateSpecialTestLabDto) {
    ...
    if (['description', 'address', 'phone_1', 'phone_2', 'email'].some(has)) permissions.push('special-tests.update');
