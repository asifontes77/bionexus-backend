...
export interface ResolvedAdmissionPayment { typePaymentId:number; currency:'BASE'|'LOCAL'; amount:number; description1:string; description2:string; }
@Injectable()
export class PatientAdmissionCloseCatalogResolver {
  async resolve(manager: EntityManager, input: NormalizedAdmissionClose) {
    ...
    const priceByExam=new Map(prices.map(price=>[price.examCatalogId,price]));
    ...
    const types=await manager.getRepository(TypePayment).find({where:{id:In(paymentIds)}});
    if(types.length!==paymentIds.length)throw new NotFoundException('PATIENT_ADMISSION_CLOSE_PAYMENT_TYPE_NOT_FOUND');
    const typeById=new Map(types.map(type=>[type.id,type]));
    const resolvedPayments:ResolvedAdmissionPayment[]=input.payments.map(item=>{const type=typeById.get(item.typePaymentId)!;if(Boolean(type.annulled))throw new BadRequestException('PATIENT_ADMISSION_CLOSE_PAYMENT_TYPE_INACTIVE');if(Boolean(type.only_dollars)&&item.currency!=='BASE')throw new BadRequestException('PATIENT_ADMISSION_CLOSE_PAYMENT_BASE_REQUIRED');if(String(type.description_1||'').trim()!==''&&item.description1==='')throw new BadRequestException('PATIENT_ADMISSION_CLOSE_PAYMENT_REFERENCE_1_REQUIRED');if(String(type.description_2||'').trim()!==''&&item.description2==='')throw new BadRequestException('PATIENT_ADMISSION_CLOSE_PAYMENT_REFERENCE_2_REQUIRED');return{...item};});
