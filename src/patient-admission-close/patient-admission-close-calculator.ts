...
export interface AdmissionCalculatedPayment { typePaymentId:number; currency:'BASE'|'LOCAL'; currencyId:number; enteredAmount:number; baseAmount:number; localAmount:number; description1:string; description2:string; }
@Injectable()
export class PatientAdmissionCloseCalculator {
  ...
  calculatePayments(payments:Array<{typePaymentId:number;currency:'BASE'|'LOCAL';currencyId:number;amount:number;description1:string;description2:string}>,exchangeRate:number){
    const rate=this.positive(exchangeRate,'PATIENT_ADMISSION_CLOSE_EXCHANGE_RATE_INVALID');return payments.map(item=>{const amount=this.positive(item.amount,'PATIENT_ADMISSION_CLOSE_PAYMENT_AMOUNT_INVALID');const baseAmount=item.currency==='BASE'?this.money(amount):this.money(amount/rate);const localAmount=item.currency==='LOCAL'?this.money(amount):this.money(amount*rate);return{typePaymentId:item.typePaymentId,currency:item.currency,currencyId:item.currencyId,enteredAmount:this.money(amount),baseAmount,localAmount,description1:item.description1,description2:item.description2};});
  }
