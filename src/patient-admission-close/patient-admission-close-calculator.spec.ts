...
it('convierte pago local a moneda base',()=>expect(service.calculatePayments([{typePaymentId:1,currency:'LOCAL',currencyId:1,amount:464,fieldValues:[]}],20)[0]).toMatchObject({baseAmount:23.2,localAmount:464}));
it('acepta cierre exactamente pagado',()=>expect(service.summarize([{examCatalogId:1,quantity:1,unitPrice:10,subtotal:10,taxRate:0,taxTotal:0,total:10}],[{typePaymentId:1,currency:'BASE',currencyId:2,enteredAmount:10,baseAmount:10,localAmount:200,fieldValues:[]}])).toMatchObject({total:10,paid:10,balance:0}));
...
it('rechaza tasa cambiaria invalida',()=>expect(()=>service.calculatePayments([{typePaymentId:1,currency:'LOCAL',currencyId:1,amount:10,fieldValues:[]}],0)).toThrow('PATIENT_ADMISSION_CLOSE_EXCHANGE_RATE_INVALID'));
