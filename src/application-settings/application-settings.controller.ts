import { Body, Controller, Get, Headers, Param, ParseIntPipe, Patch, Req, UseGuards } from '@nestjs/common';
...
export class ApplicationSettingsController {
  ...
  @Get('session/:id')
  getSessionPolicy(@Param('id', ParseIntPipe) id: number) { return this.service.getSessionPolicy(id); }
  ...
  @Patch(':id')
  update(@Req() request: SecurityAuthenticatedRequest, @Param('id', ParseIntPipe) id: number, @Body() body: UpdateApplicationSettingsDto, @Headers('x-bio-nexus-socket-id') socketId?: string) { return this.service.update(id, body, getSecurityAuditActorUserId(request) ?? undefined, socketId); }
}
